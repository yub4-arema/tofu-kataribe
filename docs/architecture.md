...
現在の会話履歴、レート制限、同時実行数、セッションキューはすべてプロセス内メモリにあります。また、音声はローカルファイルに保存します。そのため、プロセスの再起動、複数インスタンス、サーバーレス環境では状態を共有・永続化できません。

ローカルで単一プロセスを動かす場合には成立しますが、Vercel で本番運用する場合は外部の共有ストアへ置き換える必要があります。詳細は [deployment-vercel.md](./deployment-vercel.md) を参照してください。
