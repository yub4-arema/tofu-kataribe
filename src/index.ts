...
} from "./adapters/llm/openai-compatible.js";
import {
  NoSpeechProvider,
...
const llm = new OpenAiCompatibleLlmProvider(
  ...
  createOpenAiCompatibleAuth(
    ...
    process.env.OPENAI_BASIC_USERNAME,
    process.env.OPENAI_BASIC_PASSWORD,
  ),
);
const speech =
...
const server = serve({ fetch: app.fetch, port: env.port }, () =>
  console.log(
    JSON.stringify({ event: "server_started", port: env.port, speechProvider: speech.name }),
  ),
);
